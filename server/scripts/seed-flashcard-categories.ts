...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || "postgresql://postgres:postgres@localhost:5432/hospital_edu",
});
...
async function main() {
  console.log("=== Seed Flashcard Categories to Exams/Specialties/Topics ===\n");

  if (!fs.existsSync(path.join(DATA_DIR, "flashcardquestions.json"))) {
    console.error("Firebase data not found in", DATA_DIR);
    process.exit(1);
  }

  const questions = JSON.parse(fs.readFileSync(path.join(DATA_DIR, "flashcardquestions.json"), "utf-8"));
  const catDocs = JSON.parse(fs.readFileSync(path.join(DATA_DIR, "flashcardcategorys.json"), "utf-8"));
  ...
  for (const q of questions) {
    ...
    if (!specTopicCache[cacheKey]) {
      ...
      specTopicCache[cacheKey] = { specialtyId, topicId };
    }
  ...
  for (const ex of EXAMS) {
    ...
    const { rows } = await pool.query("SELECT COUNT(*)::int AS c FROM flashcards WHERE exam_id = $1 AND is_active = true", [eid]);
    const { rows: specs } = await pool.query("SELECT COUNT(*)::int AS c FROM specialties WHERE exam_id = $1", [eid]);
    console.log(`  ${ex.slug}: ${rows[0].c} flashcards, ${specs[0].c} specialties`);
  }
